...
from utils import generate_uniform_value_hist, generate_gaussian_value_hist, plot_strategy, mechanism_enc
...
def solve_player1_strategy(example, dir_name, filename, model_dir='model_ckpt/'):
    ...
    # load model
    model_name = 'MLPNet.ckpt'
    model_path = os.path.join(model_dir, model_name)
    model = MLPSolver(configs).to(device)
